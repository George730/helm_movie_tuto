...
max_for_each_type = 5000
#max_total_count = 50000

for message in consumer:
    #total_count += 1

    if (data_count >= max_for_each_type):
        break
    ...
    #    break
        ...
        # print(loads(message))
        #print(message)
        if ("recommendation request" in message) and (data_count <= max_for_each_type):
            print(message)
            time_str = message.split(',')[-1][1:-3]
            #time_str = time_str.replace(" ms", "")
            time_sum += int(time_str)
            os.system(f"echo {time_str} >> kafka_recommendation_liyan.csv")
            data_count += 1

print("Avg:", time_sum / 5000)
